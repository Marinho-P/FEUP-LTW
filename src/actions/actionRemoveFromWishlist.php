...
function removeFromWishlist($itemId) {
    ...
    $UserWishlist = Wishlist::getWishlistByUserId($db, $session->getId());
    if($UserWishlist == null){
            http_response_code(500);
            echo json_encode(array("error" => "Item not in wishlist"));
        
            exit();
    }
    else{
        $success = Wishlist::removeItemFromWishlist($db, $session->getId(),$itemId);
        if($success){
            ...
            http_response_code(500);
            echo json_encode(array("error" => "Error removing item from wishlist"));
